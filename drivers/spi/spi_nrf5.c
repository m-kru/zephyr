...
static inline nrfx_spi_bit_order_t spi_operation_to_nrfx_bit_order(
							u16_t operation)
{
...
{
	const struct spi_nrf5_cfg * const dev_cfg = DEV_CFG(spi_cfg);
...
static void spi_nrf5_nrfx_evt_handler(nrfx_spi_evt_t const *p_event,
				      void *p_context);

static int spi_nrf5_configure(struct spi_config *spi_cfg)
{
	const struct spi_nrf5_cfg * const dev_cfg = DEV_CFG(spi_cfg);
	...
	if (spi_cfg->operation & (SPI_LINES_DUAL || SPI_LINES_QUAD)) {
		SYS_LOG_ERR("SPI peripheral does not support dual "
			    "or quad mode");
		return -EINVAL;
	}
...
{
	const struct spi_nrf5_cfg * const dev_cfg =
			(struct spi_nrf5_cfg *) dev->config->config_info;
	struct spi_nrf5_data * const dev_data =
				(struct spi_nrf5_data *) dev->driver_data;
...
{
	struct spi_nrf5_data * const dev_data = DEV_DATA(spi_cfg);
	int ret;
...
static int spi_nrf5_transceive(struct spi_config *spi_cfg,
			       const struct spi_buf *tx_bufs,
			       size_t tx_count,
			       struct spi_buf *rx_bufs,
			       size_t rx_count)
...
#ifdef CONFIG_POLL
static int spi_nrf5_transceive_async(struct spi_config *spi_cfg,
				     const struct spi_buf *tx_bufs,
				     size_t tx_count,
				     struct spi_buf *rx_bufs,
				     size_t rx_count,
...
static int spi_nrf5_release(struct spi_config *spi_cfg)
{
	struct spi_nrf5_data *dev_data = DEV_DATA(spi_cfg);
...
{
	const struct spi_nrf5_cfg * const dev_cfg = dev->config->config_info;
	struct spi_nrf5_data * const dev_data = dev->driver_data;
